...
- Cierre de limitaciones (post-implentación):
  ...
  - Volquete (fase 2): las piezas de familia cajón con 'abatible'/'volquete' en nombre usan `hinge side 'inf'` (pivotante sobre bisagras inferiores, no deslizante). El signo de los lados sup/inf del eje x se corrigió para que el lado libre bascule siempre hacia +y (sup → +105°, inf → −105°). En el renderer el grupo completo (frente, lados, base, fondo, tirador) recibe la MISMA rotation con `pivot.y` en la cara frontal del frente: rotación rígida sobre el borde inferior-frontal, sin traslación rail. Doble-click en la vista 3D alterna el override de apertura de la pieza abrible (0↔1, `decideAperturaToggle`).
